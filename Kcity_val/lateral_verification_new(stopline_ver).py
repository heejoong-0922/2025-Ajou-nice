...
class pure_pursuit :
    ...
    def control_state(self, Path_state):
        ...
        if Path_state == "Global_path":
            ...
            print("taget_velocity: ", target_velocity, "Goal_velocity", goal_velocity)

        elif Path_state == "Intersection":
            self.path = self.local_path
            steering = self.calc_stanley(self.path)
            target_velocity, goal_velocity=self.control_driving_velocity(steering) #target, goal_velocity 모두 0으로 설정된다 
            brake = self.control_brake(goal_velocity) #brake =21
            print("taget_velocity: ", target_velocity, "Goal_velocity", goal_velocity)

        try :
            brake = self.park_brake()
        except :
            rospy.loginfo("Not ready Msg")
            pass
        
        steering = self.max_value(steering)
        print(target_velocity, target_velocity, target_velocity)
    ...
    def park_brake(self):
        ...
        if self.velocity/10 > self.parking_velocity+1:
            brake = 10
    ...
    def control_driving_velocity(self,steering): #goal_velocity에 도달하기 위한 target_velocity를 계산
        ...
        if self.desired_velocity == 0:
            ...
            return goal_velocity, goal_velocity
            ...
            output = self.velocity_pid(goal_velocity)
